...
Talk so a student can follow: **G-STUDENT-PRIOR** then
`references/student_prior.md`. Verbose about the Parkinson's
problem and the method; never about skill names or gates.
...
1. **Read `journal/JOURNAL.md`.** Missing/placeholder → bootstrap (§ 0).
   ...
   (EDA), History, Backlog).
2. **G-STUDENT-PRIOR.** If `Workspace decisions` has no
   `student prior:` row, fire the ask in
   `references/student_prior.md` and write the row **before** any
   modelling talk. If the row exists, use it. Talk at that level
...
- **Talk at the recorded student prior.** Session-open
  G-STUDENT-PRIOR (`references/student_prior.md`) before modelling.
  Verbose, plain language; never skill names or gates in student-
  facing text.
...
| Skip G-STUDENT-PRIOR / guess the student's level | Depth is recorded once. Ask, write the JOURNAL row, then explain |
| Jargon-first reply when `student prior:` is `beginner` | This lab vulgarizes on purpose. Define terms; say why before how |
...
- [ ] G-STUDENT-PRIOR: `student prior:` row present
...
|---|---|---|---|
| `G-STUDENT-PRIOR` | How much ML/sklearn/skore the student already has | this skill | session open, before modelling talk - see `references/student_prior.md` |
...
- `references/student_prior.md`: G-STUDENT-PRIOR ask + how to
  explain (verbose, vulgarized, adapted to the recorded level).
