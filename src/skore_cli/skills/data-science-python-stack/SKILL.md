...
- [`skore`](references/skore.md) - predictive-model evaluation built
  ...
  **Install variant per mode.** `skore.Project(...)` supports three
  ...
  extra + `load_skore_credentials()` then `login(mode="hub")` before first use; never
  browser login - see `organize-ml-workspace/references/hub_credentials.md`), `mlflow` (artifacts
