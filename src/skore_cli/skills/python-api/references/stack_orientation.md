...
- **Project**: `skore.Project(name, *, mode='local', **kwargs)`.
  ...
  - **local** (default): `skore.Project(name="<project>",
    ...
    login.
  - **hub**: `load_skore_credentials()` then `login(mode="hub")`
    then `skore.Project(name="<project>", mode="hub",
    workspace=cfg["workspace"])` (see
    `organize-ml-workspace/references/hub_credentials.md`). The
    helper finds the nearest `.skore` and sets `SKORE_HUB_URI` /
    `SKORE_HUB_API_KEY` so login uses the API key (**never**
    browser). Here `workspace=` carries the **Skore Hub org/team
    identifier** from `.skore` (a `str`, not a directory). Requires
    `pip install "skore[hub]"` and access to
