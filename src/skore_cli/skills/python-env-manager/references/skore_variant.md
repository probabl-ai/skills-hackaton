...
Hub mode loads `.skore` into `SKORE_HUB_URI` / `SKORE_HUB_API_KEY`,
then calls `skore.login(mode="hub")` before instantiating
`skore.Project(...)`. That `login` flow is implemented in the
`[hub]` extra; on a plain `skore` install,
`from skore import login` raises `ImportError`. Never use
interactive / browser login in this lab.
