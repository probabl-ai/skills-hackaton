...
| Hub mode: put `skore.login(mode="hub")` after `skore.Project(...)` | `Project(...)` constructor authenticates at init time; without prior `login`, fails. Order is fixed: load `.skore` into env, then `login`, then `Project`. Never browser login. |
...
| Hub mode: `skore.login()` auth error | Missing `.skore` / API key not exported | → `references/failure_modes.md` § "skore.login fails" - never browser login |
