...
This lab loads gitignored `.skore` (written by
`python scripts/install_skore.py`) and **never** uses
interactive / browser login. Full snippet:
`references/hub_credentials.md`.
...
| `login(mode="hub")` call | not needed | **required, before `Project(...)`, after loading `.skore` into `SKORE_HUB_URI` / `SKORE_HUB_API_KEY`** | not needed |
...
1. **Mode.** Options:
   - `local`: artifacts on disk, no account needed, recommended
     for solo work.
   - `hub`: artifacts on https://ibm.skore.probabl.ai, requires
     `.skore` credentials (no browser login), recommended for
     this lab's Submissions.
...
2. **Hub workspace name** (only when mode is `hub`). Read
   `workspace` from `.skore`: do **not** ask and do **not** open
...
|---|---|---|---|
| `<SKORE_PROJECT_INIT>` in `experiments/NN_*.py` and `audit/NN_*.py` | `skore.Project(name="<project-name>", mode="local", workspace=str(PROJECT_ROOT / "reports"))` | `load_skore_credentials()` then `login(mode="hub")` then `skore.Project(..., mode="hub", workspace=cfg["workspace"])`: see `references/hub_credentials.md` | `skore.Project(name="<experiment-name>", mode="mlflow", tracking_uri="<mlflow-tracking-uri>")` |
...
- `workspace=` is **kept**, but carries the **Hub workspace name**
  from `.skore`, not an on-disk directory.
- `name=` stays the bare project name (no slash join).
- Call `load_skore_credentials()` then `login(mode="hub")` then
  `Project(...)` in the same cell. Reuse that snippet. Never wrap
  `login`. Never interactive login.
...
- **Skore Hub account creation.** The gate assumes `.skore` already
  exists (lab `GUIDED.md`). This skill will not drive browser sign-up
  or `skore agent`.
