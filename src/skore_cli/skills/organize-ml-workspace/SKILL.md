...
| Putting `skore.login(mode="hub")` after `skore.Project(...)` | `Project(...)` requires authenticated session in hub mode. Reuse the hub_credentials snippet: `load_skore_credentials()` then `login(mode="hub")` then `Project`. Never wrap `login`. Never inline `os.environ["SKORE_HUB_*"]` |
| Inlining `json.loads(Path(".skore")...)` in experiment / audit files | Hub URI and API key come only from `src/<pkg>/hub.py` (from `templates/src_hub.py`). See `references/hub_credentials.md` |
...
- `templates/src_hub.py`: `.skore` loader (`load_skore_credentials`);
  hub mode still drops it so the import exists. Always call it
...
- `references/hub_credentials.md`: `.skore` loader; `scripts/load_skore_credentials.py`
  is the tool, copied to `src/<pkg>/hub.py` at scaffold.
